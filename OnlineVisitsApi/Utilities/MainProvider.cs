    ...
    public class MainProvider
    {
        ...
        public object Add<T>(T table)
        {
            try
            {
                ...
                else if (table.GetType() == typeof(TblPatientDoctorRel))
                {
                    ...
                    _commandText = $"insert into TblPatientDoctorRel (PatientId , DoctorId , Time , IsUp) values ('{patientDoctorRel.PatientId}' , '{patientDoctorRel.DoctorId}' , '{patientDoctorRel.Time}' , '{patientDoctorRel.IsUp}' )";
                    command = new SqlCommand($"select TOP (1) * from TblPatientDoctorRel where id = '{patientDoctorRel.id}' ORDER BY id DESC", _connection);
                    _command = new SqlCommand(_commandText, _connection);
                    _command.ExecuteNonQuery();
                    SqlDataReader reader = command.ExecuteReader();
                    reader.Read();
        ...
        public bool Update<T>(T table, int logId)
        {
            try
            {
                ...
                {
                    TblPatientDoctorRel patientDoctorRel = (TblPatientDoctorRel)tableObj;
                    _commandText = $"update TblPatientDoctorRel set PatientId = '{patientDoctorRel.PatientId}' , DoctorId = '{patientDoctorRel.DoctorId}' , Time = '{patientDoctorRel.Time}' , IsUp = '{patientDoctorRel.IsUp}' where id = '{logId}'";
                }
                _command = new SqlCommand(_commandText, _connection);
                _command.ExecuteNonQuery();
                return true;
